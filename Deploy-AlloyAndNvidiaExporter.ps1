...
.NOTES
    ...
    - WinSW exe is renamed to match the service name and placed alongside an XML config
    - See: https://github.com/winsw/winsw

    CUSTOMIZATION:
    - Update $LokiEndpoint / $PrometheusEndpoint to match your environment
    - If pre-staging binaries on a relay, update the download URL vars
#>
...
# Endpoints - CHANGE THESE to your Loki/Prometheus remote-write targets
$LokiEndpoint       = "http://YOUR_LOKI_HOST:3100/loki/api/v1/push"
$PrometheusEndpoint  = "http://YOUR_PROMETHEUS_HOST:9090/api/v1/write"

# Alloy UI listen address (0.0.0.0 = all interfaces; 127.0.0.1 = local only)
...
# Download WinSW and rename to match service name (WinSW naming convention:
# the .exe and .xml must share the same base name for auto-discovery)
...
<service>
  ...
  <name>NVIDIA GPU Prometheus Exporter</name>
...
// ---------------------------------------------------------------------------
// PROMETHEUS SCRAPE: nvidia_gpu_exporter (localhost:${GpuExporterPort})
// ---------------------------------------------------------------------------
...
prometheus.remote_write "default" {
    endpoint {
        ...
        // Uncomment and configure if auth is required:
        // basic_auth {
...
$fwRules = @(
    @{ Name = "Grafana Alloy UI";    Port = 12345;          Description = "Grafana Alloy HTTP UI" },
    @{ Name = "NVIDIA GPU Exporter"; Port = $GpuExporterPort; Description = "nvidia_gpu_exporter Prometheus metrics" }
)
...
Write-Log "Alloy UI:       http://localhost:12345"
